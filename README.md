...
**概要**
生産シミュレータ `fact_sim`（`G:\マイドライブ\プログラミング\github\fact_sim`）からエクスポートしたCSVを取り込み、Excel上でノードごとの状態遷移をタイムチャートとして表示します。1ノード1行、1秒グリッドでセル塗りつぶし表示のためスクロールが軽快です。
...
**CSVフォーマット**
`fact_sim` のエクスポートCSVで以下のヘッダを想定しています。
